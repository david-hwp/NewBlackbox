...
- [ ] **PH21-D01**: 爬虫脚本采集到的店铺订单数据必须入库，而不是只写本地 JSON 文件。
- [ ] **PH21-D02**: 每条入库订单必须关联后台 `shops.id`，并保留平台店铺 ID、店铺名称、平台、用户和渠道快照，方便后续筛选与审计。
- [ ] **PH21-D03**: 订单数据表字段必须尽可能详细地覆盖爬虫可采集信息，包括平台订单号、页面序号、订单时间、完成时间、状态、金额、顾客、联系方式、地址、商品和原始载荷。
- [ ] **PH21-D04**: 订单入库必须按 `shop_id + platform + platform_order_id` 幂等 upsert，重复采集同一订单只更新最新字段和采集时间，不生成重复记录。
- [ ] **PH21-D05**: 后端必须提供超管专用的订单列表 API，支持分页、常规筛选和按订单完成时间起止区间过滤，起止时间精确到分钟。
- [ ] **PH21-D06**: 后端必须提供受控的订单采集入库 API 或等价安全通道，让爬虫脚本能提交订单批次，并拒绝无效店铺、无效订单号和越权请求。
- [ ] **PH21-D07**: 管理后台必须新增“店铺订单”页面，只有超管可以访问和看到入口；非超管直接访问路由或 API 都应被拒绝。
- [ ] **PH21-D08**: “店铺订单”页面必须展示分页订单列表和筛选栏，筛选项包括店铺 ID、店铺名称、平台、订单状态、顾客关键字、订单号以及订单完成时间起止区间。
- [ ] **PH21-D09**: 店铺列表操作列必须增加超管可见的“店铺订单”按钮，点击后跳转到新页面并自动带上该店铺 ID、当天 00:00 到当前时刻的订单完成时间筛选条件。
- [ ] **PH21-D10**: 店铺列表的操作列必须继续固定在右侧，新增“店铺订单”按钮后不能破坏已有“远程后台”、编辑、删除操作。
- [ ] **PH21-D11**: 订单相关页面、API 和入库逻辑必须避免泄露授权 profile、cookie、token 或浏览器存储等敏感凭据；原始订单载荷只能保存业务订单内容。
- [ ] **PH21-D12**: Phase 21 必须包含后端测试、爬虫解析/提交测试、前端构建，以及内网环境超管有头浏览器验证。
...
| PH21-D01 | Phase 21 | Pending |
| PH21-D02 | Phase 21 | Pending |
| PH21-D03 | Phase 21 | Pending |
| PH21-D04 | Phase 21 | Pending |
| PH21-D05 | Phase 21 | Pending |
| PH21-D06 | Phase 21 | Pending |
| PH21-D07 | Phase 21 | Pending |
| PH21-D08 | Phase 21 | Pending |
| PH21-D09 | Phase 21 | Pending |
| PH21-D10 | Phase 21 | Pending |
| PH21-D11 | Phase 21 | Pending |
| PH21-D12 | Phase 21 | Pending |
...
*Last updated: 2026-06-24 for Phase 21 planning*
