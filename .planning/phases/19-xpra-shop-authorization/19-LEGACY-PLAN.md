...
- 2026-06-16：APP 授权窗口地址改为运行时读取后端系统参数 `app.zr.control_url` 和 `app.zr.stream_url`，APK 默认不再硬编码 ZR 控制/流地址；内网服务器 `100.99.88.2:8006` 当前系统参数为 `http://47.112.170.106:14501/` 和 `http://47.112.170.106:14500/`。
- 2026-06-16：服务端控制脚本增强多店铺会话复用健康检查：`start-zr-display.sh` 不再只按 TCP 监听判断 Xpra 就绪，会用本机 HTTP 检查并重启卡死的 Xpra；`zr-browser-control.py` 在 `/open` 返回前等待 stream HTML 可读，避免把 TCP 通但 HTTP 不返回的坏端口下发给 APP。
- 2026-06-16：小米真机 `3ca26684` 复测 `1.2.18-beta`：点击“授权失败，重新授权”后 APP 先请求 `/api/system-parameters/app` 并拿到正式公网 ZR 地址，弹窗显示本地 loading；远端 `/open` 45 秒无响应后 APP 进入失败态“授权窗口加载失败，请确认内网服务器已启动”，logcat 记录 `ShopAuthSheet` 在 `ShopAuthorizationSheetFragment.kt:261` 抛出 `SocketTimeoutException`。该结果确认 APP 运行时参数链路和失败态正常，阻塞在远端控制服务响应。
- 2026-06-16：gstack browser 验证 `http://100.99.88.2:8006/api/system-parameters/app` 返回正式 ZR 地址，`http://47.112.170.106:14501/health` 返回 200；但直接请求阿里云 `/open` 70 秒无响应，`/probe` 也超时，`14500` 不监听，`15000-15399` 未发现可返回 Xpra HTML 的端口。
- 2026-06-16：阿里云 `47.112.170.106:22` TCP 可连接但 SSH 停在 banner exchange，无法同步仓库脚本或重启 `~/data/start-zr.sh`。需要先从云控制台重启 SSH/实例，或提供可用登录通道后再部署上述脚本并恢复真实远端画面验证。
- 2026-06-16：阿里云实例恢复后确认 `~/data/start-zr.sh` 未自启动，`14501/14500/15200/15299` 均为 connection refused；手工执行 `~/data/start-zr.sh` 后 control 恢复监听 `14501`，`/health` 返回 200。
- 2026-06-16：排查到授权页打不开的直接原因是阿里云 `/etc/resolv.conf` 被 Tailscale 改写为 `100.100.100.100`，而该节点 Tailscale 离线，导致 `store.jddj.com` 无法解析，Chrome 停在 `chrome-error://chromewebdata/`。已临时改为 `223.5.5.5`、`223.6.6.6`、`8.8.8.8`，服务器可解析并 `curl -I https://store.jddj.com/base/login` 返回 200。
- 2026-06-16：同步本仓库 `admin/scripts/browser/` 到阿里云 `~/data`，修正远端仍返回 `directStreamUrl=http://100.99.88.6:<port>/` 的问题；当前 `/open` 返回 `directStreamUrl=http://47.112.170.106:15200/`，公网 `15200` 可访问 Xpra HTML。
- 2026-06-16：修复 Chromium profile stale lock。旧进程被杀后 `SingletonLock -> iZwz99idiy8ybfj6ezdfexZ-2637` 残留，导致新 Chrome 报 “profile appears to be in use” 且 `17200` CDP 不监听。`start-zr-browser.sh` 现在在确认锁内 PID 不存在时删除 `SingletonLock/SingletonSocket/SingletonCookie`，不删除 profile 数据、cookie 或 token。
- 2026-06-16：恢复验证通过。`/open?phone=15200837196&shopId=15397100...` 返回 200、`ready=true`、`alignment.ok=true`，CDP 页签为 `https://store.jddj.com/base/login`、标题“京东秒送商家端”；小米真机点击“授权失败，重新授权”后 WebView 加载 `http://47.112.170.106:15200/`，logcat 显示 Xpra WebSocket `connection-established`，截图确认弹窗内显示京东账号登录页。
